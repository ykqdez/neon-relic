...
像素版另检查 manifest 中全部二进制素材的 SHA-256、6 张图片与 25 个 PCM WAV 音效解码、中文像素字体、最近邻画布、渲染不消耗玩法随机数，以及 BGM 暂停/装备抽屉/静音和声音节点回收。游戏与测试均通过 HTTP 加载本地素材。

声音回归覆盖六武器基础/进化形态的真实攻击、独立音色与并发播放、齐射单次发声、未装备时静音、混合攻击节点预算与回收，以及 BGM 音量上限 4%。

`audio-action-checks.js` 覆盖行为触发、持续层跟随寿命、状态切换与静音回收、距离/声像、独立音频随机源、循环接缝、满预算时不互抢，以及六武器起音/持续层/危险提示的真实混音峰值，输出 `audio-actions.json`。完整验证还逐项打开独立试听页并检查开始/停止。主观听感需通过 `audio-review.html` 人工复核。

`experience-checks.js` 检查实际升级/进化选卡静音、连续选卡音乐不重启、失焦保护、重要声音抢占与压低战斗组、六武器混合输出峰值，以及脉冲刃轨迹连接、移动、淡出和不重复扣血。结果为 `experience.json`，轨迹截图为 `blade-trail-0..3.png`。峰值检查仅覆盖该自动混音场景。
...
卫星维持固定近身轨道，范围升级扩大接触面；具体热区和超频/范围组合见当前基准。相机明显滞后时的刷怪落点仍可能在实际视口内，诊断保留该结果。上述事项不应写成已完成的全盘覆盖或全场景离屏保证。
