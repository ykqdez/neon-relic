...
- **像素美术与声音**：
  - 像素冒险者、六类怪物、遗迹地板、六武器攻击特效；战场采用半分辨率画布和最近邻放大；
  - 中文像素字体、硬边面板、像素装备图标，覆盖 HUD、升级选择、暂停和战报；
  - 移动端专属性能预算（粒子上限 120 / 浮动文字 25 / 高频音频节流）；实际帧率取决于设备，尚未完成低端手机真机验证；
  - 12 个本地采样音效，六种武器各有独立攻击音色；循环 Chiptune BGM 音量为 4%，暂停、升级、查看装备或失焦时暂停音乐，`SND / OFF` 同时控制音乐与音效；
  - 二进制素材合计约 2.38 MB，无 CDN 依赖。美术与音频采用免费 CC0 素材，字体使用 SIL OFL 1.1；来源与许可见 [素材清单](assets/CREDITS.md)。
