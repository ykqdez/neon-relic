window.auditExtra=()=>{
 ...
 const sound=auditSaved.sound;sound.isMuted=false;if(sound.ctx){const ctx=sound.ctx,orig=ctx.createBufferSource.bind(ctx);let nodes=0;ctx.createBufferSource=()=>{nodes++;return orig();};sound.lastSoundTimes.hit=0;for(let i=0;i<100;i++)sound.playHit(true);const critNodes=nodes;nodes=0;sound.lastSoundTimes.hit=0;for(let i=0;i<100;i++)sound.playHit(false);out.audioThrottle={critNodes,normalNodes:nodes};ctx.createBufferSource=orig;sound.isMuted=true;}
