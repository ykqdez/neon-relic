window.experienceChecks = async () => {
  ...
  try {
    ...
    // Ordinary, elite and Boss deaths use the new crumble sound, separate from explosions.
    window.soundSystem=sound;sound.isMuted=false;
    const deathCalls=[];
    sound.playSample=function(name,options){deathCalls.push({name,...options});return sample.call(this,name,options);};
    for(const type of ['normal','elite','boss']){
      const enemy=dummy(100,0);enemy.hp=1;enemy.isElite=type==='elite';enemy.isBoss=type==='boss';
      const count=deathCalls.length;enemy.takeDamage(10,false,quietPool());enemy.takeDamage(10,false,quietPool());
      check(deathCalls.length===count+1&&deathCalls[count].name==='enemy-shatter',`one new death cue: ${type}`);
    }
    sound.playSample=sample;window.soundSystem=null;
    const cannon=new WeaponRegistry.plasma_cannon();cannon.level=1;
    const plasmaCanvas=document.createElement('canvas');plasmaCanvas.width=460;plasmaCanvas.height=240;
    const plasmaCtx=plasmaCanvas.getContext('2d');plasmaCtx.fillStyle='#151c2a';plasmaCtx.fillRect(0,0,460,240);
    for(const [evolved,y] of [[false,65],[true,175]]){
      const demo=new WeaponRegistry.plasma_cannon(),pilot=new Player(30,y);demo.level=5;if(evolved)demo.evolve();
      demo.fire(pilot,[],quietPool());demo.timer=999;for(let i=0;i<45;i++)demo.update(1/60,pilot,[],quietPool());
      demo.render(plasmaCtx);PixelArt.player(plasmaCtx,pilot);
    }
    window.plasmaPreview=plasmaCanvas.toDataURL();
    cannon.fire(new Player(0,0),[],quietPool());cannon.timer=999;
    const target=dummy(36,0,4);cannon.update(.1,new Player(0,0),[target],quietPool());
    check(cannon.impacts.length===1&&cannon.muzzleFlashes.length===1&&cannon.projectiles[0].age===.1,'plasma muzzle, impact and moving orb generated by actual attack');
    for(let i=0;i<40;i++)cannon.fire(new Player(0,0),[],quietPool());
    cannon.update(.01,new Player(0,0),[dummy(4,0,100)],quietPool());
    check(cannon.muzzleFlashes.length<=6&&cannon.impacts.length<=24,'plasma visual pools stay bounded');
    cannon.timer=999;cannon.update(4,new Player(0,0),[],quietPool());
    check(cannon.impacts.length===0&&cannon.muzzleFlashes.length===0&&cannon.projectiles.length===0,'plasma visuals expire');
    const fetchSample=window.fetch;
    try {
      sound.buffers.delete('laser1');
      window.fetch=(url,...args)=>String(url).endsWith('/laser1.wav')?Promise.resolve(new Response('temporary',{status:503})):fetchSample(url,...args);
      await sound.loadSamples();
      check(sound.failed.includes('laser1')&&!sound.buffers.has('laser1'),'temporary sample failure is recorded');
      window.fetch=fetchSample;sound.lastLoadAttempt=-Infinity;sound.unlock();await sound.ready;
      check(sound.buffers.has('laser1')&&!sound.failed.includes('laser1'),'later interaction retries failed PCM sample');
    } finally {window.fetch=fetchSample;}
