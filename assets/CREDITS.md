...
| Plasma shot / Enemy shatter | 本项目 [生成脚本](../tools/build_audio.py) | CC0 1.0 | 原创合成的能量炮发射声与敌人碎裂消散声，预先生成为 WAV，不在战斗中合成。 |
...
- 转码说明与两项原创合成音效许可：`licenses/generated-sfx.txt`。离线重新生成需要 Python 与 soundfile，执行 `python tools/build_audio.py`；游戏运行不需要 Python 或额外依赖。
