...
- 武器每种最多 2 声（包含起音和持续层），所有武器合计最多 8 声；普通战斗组最多 12 声，重要提示组最多 4 声，全局另有 20 声硬上限。
- 结算 > Boss 警示 > 受伤 > Boss 死亡 > 敌人预警 > 大爆炸 > 进化攻击 > 基础攻击 > 持续层 > 小爆炸 > 碰撞 > 普通死亡 > 命中 > 拾取。达到上限时按所在分组淘汰更低优先级、同级更早的声音；持续层只可抢占更低优先级，避免两个循环声互相重启。玩家超新星属于普通战斗组，不触发危险提示压低。
...
脉冲刃使用 0.18 秒空气挥切与轻金属边缘声；棱镜射线的 0.48 秒起音叠加低音量周期层，周期层跟随实际光束寿命并在结束时淡出。黑洞同样分启动与持续层，使用独立引力音色。多条光束/多个黑洞按武器合并持续声，计入原有声部预算。素材由 tools/build_audio.py 离线生成 PCM WAV，许可 CC0。

玩家受伤使用独立机体冲击；等离子非致命命中补短碰撞；敌人蓄力和 Boss 危险圈各响一次预警；晶体拾取缩为 75ms、90ms 限频。部分短音使用独立随机源微调音高；世界命中/死亡/引力/爆炸声按距离衰减，左右声像幅度有限。暂停、静音与重开清除旧声；升级保留 BGM。完整检查表、行业参考与试听入口见 [行为音效检查](AUDIO_AUDIT.md)，战斗数值见 [战斗设计](COMBAT_DESIGN.md)。
